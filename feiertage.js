module.exports = function(RED) {
    function feiertage(config) {
        ...
        var Feiertage;
        try {
            Feiertage = require('getfeiertag42').Feiertage;
        } catch (error) {
        ...
        var checkBundesfeierCH = config.bundesfeierCH; // checkbox Bundesfeier
        var bundesfeierCHName = config.bundesfeierCHName; // Bundesfeier CH Name
        ...
        newYear[2] = Feiertage.getNeujahr(formatDateObj, currentYear);
        newYear[3] = Feiertage.getNeujahr(formatDE, currentYear);
        var berchtoldstag = []; // day of Berchtoldstag
        berchtoldstag[0] = "Berchtoldstag"; // id
        berchtoldstag[1] = berchtoldstagName; // name
        berchtoldstag[2] = Feiertage.getBerchtoldstag(formatDateObj, currentYear);
        berchtoldstag[3] = Feiertage.getBerchtoldstag(formatDE, currentYear);
        var epiphany = []; // day of Holy Three Kings
        epiphany[0] = "Epiphany"; // id
        epiphany[1] = epiphanyName; // name
        epiphany[2] = Feiertage.getHeiligeDreiKoenige(formatDateObj, currentYear);
        epiphany[3] = Feiertage.getHeiligeDreiKoenige(formatDE, currentYear);
        var weiberfastnacht = []; // day of Weiberfastnacht
        weiberfastnacht[0] = "Weiberfastnacht"; // id
        weiberfastnacht[1] = weiberfastnachtName; // name
        weiberfastnacht[2] = Feiertage.getWeiberfastnacht(formatDateObj, currentYear);
        weiberfastnacht[3] = Feiertage.getWeiberfastnacht(formatDE, currentYear);
        var valentinstag = []; // day of Valentinstag
        valentinstag[0] = "Valentinstag"; // id
        valentinstag[1] = valentinstagName; // name
        valentinstag[2] = Feiertage.getValentinstag(formatDateObj, currentYear);
        valentinstag[3] = Feiertage.getValentinstag(formatDE, currentYear);
        var rosenmontag = []; // day of Rosenmontag
        rosenmontag[0] = "Rosenmontag"; // id
        rosenmontag[1] = rosenmontagName; // name
        rosenmontag[2] = Feiertage.getRosenmontag(formatDateObj, currentYear);
        rosenmontag[3] = Feiertage.getRosenmontag(formatDE, currentYear);
        ...
        fastnachtsdienstag[2] = Feiertage.getFastnachtsdienstag(formatDateObj, currentYear);
        fastnachtsdienstag[3] = Feiertage.getFastnachtsdienstag(formatDE, currentYear);
        var aschermittwoch = []; // day of Aschermittwoch
        aschermittwoch[0] = "Aschermittwoch"; // id
        aschermittwoch[1] = aschermittwochName; // name
        aschermittwoch[2] = Feiertage.getAschermittwoch(formatDateObj, currentYear);
        aschermittwoch[3] = Feiertage.getAschermittwoch(formatDE, currentYear);
        var stJosef = []; // day of St. Josef
        stJosef[0] = "St. Josef"; // id
        stJosef[1] = stJosefName; // name
        stJosef[2] = Feiertage.getStJosef(formatDateObj, currentYear);
        stJosef[3] = Feiertage.getStJosef(formatDE, currentYear);
        var gruendonnerstag = []; // day of Gründonnerstag
        gruendonnerstag[0] = "Gründonnerstag"; // id
        gruendonnerstag[1] = gruendonnerstagName; // name
        gruendonnerstag[2] = Feiertage.getGruendonnerstag(formatDateObj, currentYear);
        gruendonnerstag[3] = Feiertage.getGruendonnerstag(formatDE, currentYear);
        var karfreitag = []; // day of Karfreitag
        karfreitag[0] = "Karfreitag"; // id
        karfreitag[1] = karfreitagName; // name
        karfreitag[2] = Feiertage.getKarfreitag(formatDateObj, currentYear);
        karfreitag[3] = Feiertage.getKarfreitag(formatDE, currentYear);
        var easterSunday = []; // day of Easter Sunday
        easterSunday[0] = "Easter Sunday"; // id
        easterSunday[1] = easterSundayName; // name
        easterSunday[2] = Feiertage.getOstersonntag(formatDateObj, currentYear);
        easterSunday[3] = Feiertage.getOstersonntag(formatDE, currentYear);
        var easterMonday = []; // day of easter Sunday
        easterMonday[0] = "Easter Monday"; // id
        easterMonday[1] = easterMondayName; // name
        easterMonday[2] = Feiertage.getOstermontag(formatDateObj, currentYear);
        easterMonday[3] = Feiertage.getOstermontag(formatDE, currentYear);
        var firstMay = []; // day of First May
        firstMay[0] = "First May"; // id
        firstMay[1] = firstMayName; // name
        firstMay[2] = Feiertage.getFirstMay(formatDateObj, currentYear);
        firstMay[3] = Feiertage.getFirstMay(formatDE, currentYear);
        var stFlorian = []; // day of St. Florian
        stFlorian[0] = "St. Florian"; // id
        stFlorian[1] = stFlorianName; // name
        stFlorian[2] = Feiertage.getStFlorian(formatDateObj, currentYear);
        stFlorian[3] = Feiertage.getStFlorian(formatDE, currentYear);
        ...
        christiHimmelfahrt[2] = Feiertage.getChristiHimmelfahrt(formatDateObj, currentYear);
        christiHimmelfahrt[3] = Feiertage.getChristiHimmelfahrt(formatDE, currentYear);
        var pfingstsonntag = []; // day of Pfingstsonntag
        pfingstsonntag[0] = "Pfingstsonntag"; // id
        pfingstsonntag[1] = pfingstsonntagName; // name
        pfingstsonntag[2] = Feiertage.getPfingstsonntag(formatDateObj, currentYear);
        pfingstsonntag[3] = Feiertage.getPfingstsonntag(formatDE, currentYear);
        var pfingstmontag = []; // day of Pfingstmontag
        pfingstmontag[0] = "Pfingstmontag"; // id
        pfingstmontag[1] = pfingstmontagName; // name
        pfingstmontag[2] = Feiertage.getPfingstmontag(formatDateObj, currentYear);
        pfingstmontag[3] = Feiertage.getPfingstmontag(formatDE, currentYear);
        var fronleichnam = []; // day of Fronleichnam
        fronleichnam[0] = "Fronleichnam"; // id
        fronleichnam[1] = fronleichnamName; // name
        fronleichnam[2] = Feiertage.getFronleichnam(formatDateObj, currentYear);
        fronleichnam[3] = Feiertage.getFronleichnam(formatDE, currentYear);
        var bundesfeierCH = []; // day of Bundesfeier CH
        bundesfeierCH[0] = "Bundesfeier CH"; // id
        bundesfeierCH[1] = bundesfeierCHName; // name
        bundesfeierCH[2] = Feiertage.getBundesfeierCH(formatDateObj, currentYear);
        bundesfeierCH[3] = Feiertage.getBundesfeierCH(formatDE, currentYear);
        ...
        mariaHimmelfahrt[2] = Feiertage.getMariaHimmelfahrt(formatDateObj, currentYear);
        mariaHimmelfahrt[3] = Feiertage.getMariaHimmelfahrt(formatDE, currentYear);
        var stRupert = []; // day of St. Rupert
        stRupert[0] = "St. Rupert"; // id
        stRupert[1] = stRupertName; // name
        stRupert[2] = Feiertage.getStRupert(formatDateObj, currentYear);
        stRupert[3] = Feiertage.getStRupert(formatDE, currentYear);
        ...
        tagDerDeutschenEinheit[2] = Feiertage.getTagDerDeutschenEinheit(formatDateObj, currentYear);
        tagDerDeutschenEinheit[3] = Feiertage.getTagDerDeutschenEinheit(formatDE, currentYear);
        var tagDerVolksabstimmung = []; // day of Tag der Volksabstimmung
        tagDerVolksabstimmung[0] = "Tag der Volksabstimmung AT"; // id
        tagDerVolksabstimmung[1] = tagDerVolksabstimmungName; // name
        tagDerVolksabstimmung[2] = Feiertage.getTagDerVolksabstimmung(formatDateObj, currentYear);
        tagDerVolksabstimmung[3] = Feiertage.getTagDerVolksabstimmung(formatDE, currentYear);
        var nationalfeiertagAT = []; // day of Nationalfeiertag AT
        nationalfeiertagAT[0] = "Nationalfeiertag AT"; // id
        nationalfeiertagAT[1] = nationalfeiertagATName; // name
        nationalfeiertagAT[2] = Feiertage.getNationalfeiertagAT(formatDateObj, currentYear);
        nationalfeiertagAT[3] = Feiertage.getNationalfeiertagAT(formatDE, currentYear);
        var halloween = []; // day of Halloween
        halloween[0] = "Halloween"; // id
        halloween[1] = halloweenName; // name
        halloween[2] = Feiertage.getHalloween(formatDateObj, currentYear);
        halloween[3] = Feiertage.getHalloween(formatDE, currentYear);
        var allerheiligen = []; // day of Allerheiligen
        allerheiligen[0] = "Allerheiligen"; // id
        allerheiligen[1] = allerheiligenName; // name
        allerheiligen[2] = Feiertage.getAllerheiligen(formatDateObj, currentYear);
        allerheiligen[3] = Feiertage.getAllerheiligen(formatDE, currentYear);
        var stMartin = []; // day of St. Martin
        stMartin[0] = "St. Martin"; // id
        stMartin[1] = stMartinName; // name
        stMartin[2] = Feiertage.getStMartin(formatDateObj, currentYear);
        stMartin[3] = Feiertage.getStMartin(formatDE, currentYear);
        var stLeopold = []; // day of St. Leopold
        stLeopold[0] = "St. Leopold"; // id
        stLeopold[1] = stLeopoldName; // name
        stLeopold[2] = Feiertage.getStLeopold(formatDateObj, currentYear);
        stLeopold[3] = Feiertage.getStLeopold(formatDE, currentYear);
        var bussUndBettag = []; // day of Buß und Bettag
        bussUndBettag[0] = "Buß und Bettag"; // id
        bussUndBettag[1] = bussUndBettagName; // name
        bussUndBettag[2] = Feiertage.getBussUndBettag(formatDateObj, currentYear);
        bussUndBettag[3] = Feiertage.getBussUndBettag(formatDE, currentYear);
        var santa = []; // day of Nikolaus
        santa[0] = "Santa Day"; // id
        santa[1] = santaName; // name
        santa[2] = Feiertage.getNikolaus(formatDateObj, currentYear);
        santa[3] = Feiertage.getNikolaus(formatDE, currentYear);
        ...
        mariaeEmpfaengnis[2] = Feiertage.getMariaeEmpfaengnis(formatDateObj, currentYear);
        mariaeEmpfaengnis[3] = Feiertage.getMariaeEmpfaengnis(formatDE, currentYear);
        var advent1 = []; // day of first Advent
        advent1[0] = "1. Advent"; // id
        advent1[1] = advent1Name; // name
        advent1[2] = Feiertage.getAdvent1(formatDateObj, currentYear);
        advent1[3] = Feiertage.getAdvent1(formatDE, currentYear);
        var advent2 = []; // day of second Advent
        advent2[0] = "2. Advent"; // id
        advent2[1] = advent2Name; // name
        advent2[2] = Feiertage.getAdvent2(formatDateObj, currentYear);
        advent2[3] = Feiertage.getAdvent2(formatDE, currentYear);
        var advent3 = []; // day of third Advent
        advent3[0] = "3. Advent"; // id
        advent3[1] = advent3Name; // name
        advent3[2] = Feiertage.getAdvent3(formatDateObj, currentYear);
        advent3[3] = Feiertage.getAdvent3(formatDE, currentYear);
        var advent4 = []; // day of fourth Advent
        advent4[0] = "4. Advent"; // id
        advent4[1] = advent4Name; // name
        advent4[2] = Feiertage.getAdvent4(formatDateObj, currentYear);
        advent4[3] = Feiertage.getAdvent4(formatDE, currentYear);
        var christmasEve = []; // day of Christmas Eve
        christmasEve[0] = "Christmas Eve"; // id
        christmasEve[1] = christmasEveName; // name
        christmasEve[2] = Feiertage.getHeiligabend(formatDateObj, currentYear);
        christmasEve[3] = Feiertage.getHeiligabend(formatDE, currentYear);
        ...
        firstDayChristmas[2] = Feiertage.getWeihnachtsfeiertag1(formatDateObj, currentYear);
        firstDayChristmas[3] = Feiertage.getWeihnachtsfeiertag1(formatDE, currentYear);
        ...
        secondDayChristmas[2] = Feiertage.getWeihnachtsfeiertag2(formatDateObj, currentYear);
        secondDayChristmas[3] = Feiertage.getWeihnachtsfeiertag2(formatDE, currentYear);
        var newYearsEve = []; // day of New Years Eve
        newYearsEve[0] = "New Years Eve"; // id
        newYearsEve[1] = newYearsEveName; // name
        newYearsEve[2] = Feiertage.getSilvester(formatDateObj, currentYear);
        newYearsEve[3] = Feiertage.getSilvester(formatDE, currentYear);
        ...
        function refreshHoliday() {
            ...
            if ((new Date(newYear[2]) - new Date(currentYear + "-" + currentMonth + "-" + currentDay)) < 0) {
                newYear[2] = Feiertage.getNeujahr(formatDateObj, currentYear + 1);
                newYear[3] = Feiertage.getNeujahr(formatDE, currentYear + 1);
            }
            if ((new Date(berchtoldstag[2]) - new Date(currentYear + "-" + currentMonth + "-" + currentDay)) < 0) {
                berchtoldstag[2] = Feiertage.getBerchtoldstag(formatDateObj, currentYear + 1);
                berchtoldstag[3] = Feiertage.getBerchtoldstag(formatDE, currentYear + 1);
            }
            if ((new Date(epiphany[2]) - new Date(currentYear + "-" + currentMonth + "-" + currentDay)) < 0) {
                epiphany[2] = Feiertage.getHeiligeDreiKoenige(formatDateObj, currentYear + 1);
                epiphany[3] = Feiertage.getHeiligeDreiKoenige(formatDE, currentYear + 1);
            }
            
            if ((new Date(weiberfastnacht[2]) - new Date(currentYear + "-" + currentMonth + "-" + currentDay)) < 0) {
                weiberfastnacht[2] = Feiertage.getWeiberfastnacht(formatDateObj, currentYear + 1);
                weiberfastnacht[3] = Feiertage.getWeiberfastnacht(formatDE, currentYear + 1);
            }
            if ((new Date(valentinstag[2]) - new Date(currentYear + "-" + currentMonth + "-" + currentDay)) < 0) {
                valentinstag[2] = Feiertage.getValentinstag(formatDateObj, currentYear + 1);
                valentinstag[3] = Feiertage.getValentinstag(formatDE, currentYear + 1);
            }
            if ((new Date(rosenmontag[2]) - new Date(currentYear + "-" + currentMonth + "-" + currentDay)) < 0) {
                rosenmontag[2] = Feiertage.getRosenmontag(formatDateObj, currentYear + 1);
                rosenmontag[3] = Feiertage.getRosenmontag(formatDE, currentYear + 1);
            }
            if ((new Date(fastnachtsdienstag[2]) - new Date(currentYear + "-" + currentMonth + "-" + currentDay)) < 0) {
                fastnachtsdienstag[2] = Feiertage.getFastnachtsdienstag(formatDateObj, currentYear + 1);
                fastnachtsdienstag[3] = Feiertage.getFastnachtsdienstag(formatDE, currentYear + 1);
            }
            if ((new Date(aschermittwoch[2]) - new Date(currentYear + "-" + currentMonth + "-" + currentDay)) < 0) {
                aschermittwoch[2] = Feiertage.getAschermittwoch(formatDateObj, currentYear + 1);
                aschermittwoch[3] = Feiertage.getAschermittwoch(formatDE, currentYear + 1);
            }
            if ((new Date(stJosef[2]) - new Date(currentYear + "-" + currentMonth + "-" + currentDay)) < 0) {
                stJosef[2] = Feiertage.getStJosef(formatDateObj, currentYear + 1);
                stJosef[3] = Feiertage.getStJosef(formatDE, currentYear + 1);
            }
            if ((new Date(gruendonnerstag[2]) - new Date(currentYear + "-" + currentMonth + "-" + currentDay)) < 0) {
                gruendonnerstag[2] = Feiertage.getGruendonnerstag(formatDateObj, currentYear + 1);
                gruendonnerstag[3] = Feiertage.getGruendonnerstag(formatDE, currentYear + 1);
            }
            if ((new Date(karfreitag[2]) - new Date(currentYear + "-" + currentMonth + "-" + currentDay)) < 0) {
                karfreitag[2] = Feiertage.getKarfreitag(formatDateObj, currentYear + 1);
                karfreitag[3] = Feiertage.getKarfreitag(formatDE, currentYear + 1);
            }
            if ((new Date(easterSunday[2]) - new Date(currentYear + "-" + currentMonth + "-" + currentDay)) < 0) {
                easterSunday[2] = Feiertage.getOstersonntag(formatDateObj, currentYear + 1);
                easterSunday[3] = Feiertage.getOstersonntag(formatDE, currentYear + 1);
            }
            if ((new Date(easterMonday[2]) - new Date(currentYear + "-" + currentMonth + "-" + currentDay)) < 0) {
                easterMonday[2] = Feiertage.getOstermontag(formatDateObj, currentYear + 1);
                easterMonday[3] = Feiertage.getOstermontag(formatDE, currentYear + 1);
            }
            if ((new Date(firstMay[2]) - new Date(currentYear + "-" + currentMonth + "-" + currentDay)) < 0) {
                firstMay[2] = Feiertage.getFirstMay(formatDateObj, currentYear + 1);
                firstMay[3] = Feiertage.getFirstMay(formatDE, currentYear + 1);
            }
            if ((new Date(stFlorian[2]) - new Date(currentYear + "-" + currentMonth + "-" + currentDay)) < 0) {
                stFlorian[2] = Feiertage.getStFlorian(formatDateObj, currentYear + 1);
                stFlorian[3] = Feiertage.getStFlorian(formatDE, currentYear + 1);
            }
            if ((new Date(christiHimmelfahrt[2]) - new Date(currentYear + "-" + currentMonth + "-" + currentDay)) < 0) {
                christiHimmelfahrt[2] = Feiertage.getChristiHimmelfahrt(formatDateObj, currentYear + 1);
                christiHimmelfahrt[3] = Feiertage.getChristiHimmelfahrt(formatDE, currentYear + 1);
            }
            if ((new Date(pfingstsonntag[2]) - new Date(currentYear + "-" + currentMonth + "-" + currentDay)) < 0) {
                pfingstsonntag[2] = Feiertage.getPfingstsonntag(formatDateObj, currentYear + 1);
                pfingstsonntag[3] = Feiertage.getPfingstsonntag(formatDE, currentYear + 1);
            }
            if ((new Date(pfingstmontag[2]) - new Date(currentYear + "-" + currentMonth + "-" + currentDay)) < 0) {
                pfingstmontag[2] = Feiertage.getPfingstmontag(formatDateObj, currentYear + 1);
                pfingstmontag[3] = Feiertage.getPfingstmontag(formatDE, currentYear + 1);
            }
            if ((new Date(fronleichnam[2]) - new Date(currentYear + "-" + currentMonth + "-" + currentDay)) < 0) {
                fronleichnam[2] = Feiertage.getFronleichnam(formatDateObj, currentYear + 1);
                fronleichnam[3] = Feiertage.getFronleichnam(formatDE, currentYear + 1);
            }
            if ((new Date(bundesfeierCH[2]) - new Date(currentYear + "-" + currentMonth + "-" + currentDay)) < 0) {
                bundesfeierCH[2] = Feiertage.getBundesfeierCH(formatDateObj, currentYear + 1);
                bundesfeierCH[3] = Feiertage.getBundesfeierCH(formatDE, currentYear + 1);
            }
            if ((new Date(mariaHimmelfahrt[2]) - new Date(currentYear + "-" + currentMonth + "-" + currentDay)) < 0) {
                mariaHimmelfahrt[2] = Feiertage.getMariaHimmelfahrt(formatDateObj, currentYear + 1);
                mariaHimmelfahrt[3] = Feiertage.getMariaHimmelfahrt(formatDE, currentYear + 1);
            }
            if ((new Date(stRupert[2]) - new Date(currentYear + "-" + currentMonth + "-" + currentDay)) < 0) {
                stRupert[2] = Feiertage.getStRupert(formatDateObj, currentYear + 1);
                stRupert[3] = Feiertage.getStRupert(formatDE, currentYear + 1);
            }
            if ((new Date(tagDerDeutschenEinheit[2]) - new Date(currentYear + "-" + currentMonth + "-" + currentDay)) < 0) {
                tagDerDeutschenEinheit[2] = Feiertage.getTagDerDeutschenEinheit(formatDateObj, currentYear + 1);
                tagDerDeutschenEinheit[3] = Feiertage.getTagDerDeutschenEinheit(formatDE, currentYear + 1);
            }
            if ((new Date(tagDerVolksabstimmung[2]) - new Date(currentYear + "-" + currentMonth + "-" + currentDay)) < 0) {
                tagDerVolksabstimmung[2] = Feiertage.getTagDerVolksabstimmung(formatDateObj, currentYear + 1);
                tagDerVolksabstimmung[3] = Feiertage.getTagDerVolksabstimmung(formatDE, currentYear + 1);
            }
            if ((new Date(nationalfeiertagAT[2]) - new Date(currentYear + "-" + currentMonth + "-" + currentDay)) < 0) {
                nationalfeiertagAT[2] = Feiertage.getNationalfeiertagAT(formatDateObj, currentYear + 1);
                nationalfeiertagAT[3] = Feiertage.getNationalfeiertagAT(formatDE, currentYear + 1);
            }
            if ((new Date(halloween[2]) - new Date(currentYear + "-" + currentMonth + "-" + currentDay)) < 0) {
                halloween[2] = Feiertage.getHalloween(formatDateObj, currentYear + 1);
                halloween[3] = Feiertage.getHalloween(formatDE, currentYear + 1);
            }
            if ((new Date(allerheiligen[2]) - new Date(currentYear + "-" + currentMonth + "-" + currentDay)) < 0) {
                allerheiligen[2] = Feiertage.getAllerheiligen(formatDateObj, currentYear + 1);
                allerheiligen[3] = Feiertage.getAllerheiligen(formatDE, currentYear + 1);
            }
            if ((new Date(stMartin[2]) - new Date(currentYear + "-" + currentMonth + "-" + currentDay)) < 0) {
                stMartin[2] = Feiertage.getStMartin(formatDateObj, currentYear + 1);
                stMartin[3] = Feiertage.getStMartin(formatDE, currentYear + 1);
            }
            if ((new Date(stLeopold[2]) - new Date(currentYear + "-" + currentMonth + "-" + currentDay)) < 0) {
                stLeopold[2] = Feiertage.getStLeopold(formatDateObj, currentYear + 1);
                stLeopold[3] = Feiertage.getStLeopold(formatDE, currentYear + 1);
            }
            if ((new Date(bussUndBettag[2]) - new Date(currentYear + "-" + currentMonth + "-" + currentDay)) < 0) {
                bussUndBettag[2] = Feiertage.getBussUndBettag(formatDateObj, currentYear + 1);
                bussUndBettag[3] = Feiertage.getBussUndBettag(formatDE, currentYear + 1);
            }
            if ((new Date(santa[2]) - new Date(currentYear + "-" + currentMonth + "-" + currentDay)) < 0) {
                santa[2] = Feiertage.getNikolaus(formatDateObj, currentYear + 1);
                santa[3] = Feiertage.getNikolaus(formatDE, currentYear + 1);
            }
            if ((new Date(mariaeEmpfaengnis[2]) - new Date(currentYear + "-" + currentMonth + "-" + currentDay)) < 0) {
                mariaeEmpfaengnis[2] = Feiertage.getMariaeEmpfaengnis(formatDateObj, currentYear + 1);
                mariaeEmpfaengnis[3] = Feiertage.getMariaeEmpfaengnis(formatDE, currentYear + 1);
            }
            if ((new Date(advent1[2]) - new Date(currentYear + "-" + currentMonth + "-" + currentDay)) < 0) {
                advent1[2] = Feiertage.getAdvent1(formatDateObj, currentYear + 1);
                advent1[3] = Feiertage.getAdvent1(formatDE, currentYear + 1);
            }
            if ((new Date(advent2[2]) - new Date(currentYear + "-" + currentMonth + "-" + currentDay)) < 0) {
                advent2[2] = Feiertage.getAdvent2(formatDateObj, currentYear + 1);
                advent2[3] = Feiertage.getAdvent2(formatDE, currentYear + 1);
            }
            if ((new Date(advent3[2]) - new Date(currentYear + "-" + currentMonth + "-" + currentDay)) < 0) {
                advent3[2] = Feiertage.getAdvent3(formatDateObj, currentYear + 1);
                advent3[3] = Feiertage.getAdvent3(formatDE, currentYear + 1);
            }
            if ((new Date(advent4[2]) - new Date(currentYear + "-" + currentMonth + "-" + currentDay)) < 0) {
                advent4[2] = Feiertage.getAdvent4(formatDateObj, currentYear + 1);
                advent4[3] = Feiertage.getAdvent4(formatDE, currentYear + 1);
            }
            if ((new Date(christmasEve[2]) - new Date(currentYear + "-" + currentMonth + "-" + currentDay)) < 0) {
                christmasEve[2] = Feiertage.getHeiligabend(formatDateObj, currentYear + 1);
                christmasEve[3] = Feiertage.getHeiligabend(formatDE, currentYear + 1);
            }
            if ((new Date(firstDayChristmas[2]) - new Date(currentYear + "-" + currentMonth + "-" + currentDay)) < 0) {
                firstDayChristmas[2] = Feiertage.getWeihnachtsfeiertag1(formatDateObj, currentYear + 1);
                firstDayChristmas[3] = Feiertage.getWeihnachtsfeiertag1(formatDE, currentYear + 1);
            }
            if ((new Date(secondDayChristmas[2]) - new Date(currentYear + "-" + currentMonth + "-" + currentDay)) < 0) {
                secondDayChristmas[2] = Feiertage.getWeihnachtsfeiertag2(formatDateObj, currentYear + 1);
                secondDayChristmas[3] = Feiertage.getWeihnachtsfeiertag2(formatDE, currentYear + 1);
            }
            if ((new Date(newYearsEve[2]) - new Date(currentYear + "-" + currentMonth + "-" + currentDay)) < 0) {
                newYearsEve[2] = Feiertage.getSilvester(formatDateObj, currentYear + 1);
                newYearsEve[3] = Feiertage.getSilvester(formatDE, currentYear + 1);
            }
        ...
        function checkbox() {
            ...
            if (checkBundesfeierCH) {
                ...
                var index = holiday.indexOf(bundesfeierCH); // get index of item
                if (index >= 0) {
                    holiday.splice(index); // remove item at index
                }
            }
